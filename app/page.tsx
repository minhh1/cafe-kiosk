...
export default function MartinPlaceDashboard() {
  // --- HELPERS (Defined first to avoid reference errors) ---
  const getTodayDate = () => {
    const d = new Date();
  ...
  const getLocalDateString = (ts: number | string) => {
    if (typeof ts === 'string') return ts; // If manual date
  ...
  // --- ACTIONS ---
  const handleConfirmCoffee = useCallback(async () => {
    setIsSaving(true);
  ...
  }, [selectedDate, coffeeStandard, coffeeExtra, coffeeOperator, isCoffeeConfirmed]);
  ...
  const submitAttendance = async (shift: any, type: string) => {
    ...
    setIsSaving(true);
    const newTime = type.startsWith('EDIT') ? format24to12(formTime) : null;

    const { error } = await supabase.from('attendance_logs').insert({
      shift_id: shift.Id.toString(), staff_name: shift._DPMetaData?.EmployeeInfo?.DisplayName,
      action_type: finalType, notes: (finalType === 'OTHER' || finalType === 'NO_SHOW') ? formNote : "",
      override_start: type === 'EDIT_START' ? newTime : null, override_end: type === 'EDIT_END' ? newTime : null
    });

    if (!error) { 
        setActiveForm(null); setFormNote(""); setFormTime(""); 
        await fetchData(true); // Targeted refresh of logs ONLY
    }
    setIsSaving(false);
  };
        ...
        {viewMode === 'weekly' && (
          <div className="animate-in slide-in-from-top-4 duration-500 text-left">
            <div className="hidden print:block border-b-2 border-slate-900 pb-1 mb-2 text-left">
                ...
                <p className="text-[7pt] font-bold text-left">Week: {new Date(weekRange.mon).toLocaleDateString()} — {new Date(weekRange.sun).toLocaleDateString()} | Total Weekly Coffee: {confirmedWeeklyCoffeeTotal.toFixed(1)}kg</p>
            ...
            <div className="mb-4 bg-slate-800 text-white p-6 rounded-3xl shadow-lg flex justify-between items-center print:hidden text-left">
               ...
               <div className="text-right"><p className="text-[10px] font-black text-orange-400 uppercase mb-1">Weekly Coffee Total</p><p className="text-4xl font-black text-orange-500">{confirmedWeeklyCoffeeTotal.toFixed(1)} <span className="text-sm uppercase">KG</span></p></div>
            </div>
            ...
            <div className="flex justify-end gap-2 mb-4 print:hidden text-left"><button onClick={exportToCSV} className="bg-slate-100 text-slate-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase hover:bg-slate-200 transition-colors">CSV</button><button onClick={() => window.print()} className="bg-orange-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase shadow-md">Print PDF</button></div>
          </div>
        )}
            ...
            <div className={`border-t-4 ${team.col.replace('bg-', 'border-')} pt-6 print:border-none print:pt-1`}>
                {Array.from(new Set(shifts.filter(s => s._DPMetaData?.OperationalUnitInfo?.OperationalUnitName === team.id).map(s => s._DPMetaData.EmployeeInfo.DisplayName))).sort().map(empName => {
                    ...
                    if (viewMode === 'daily') {
                            ...
                            return (
                                <div key={s.Id} className={`bg-white p-5 rounded-2xl shadow-sm border mb-4 ${log ? STATUS_COLORS[log.action_type].border : 'border-slate-200'}`}>
                                    <div className="flex justify-between items-center gap-4 text-left">
                                        <div className="flex-1 text-left"><div className="flex items-center gap-3 text-left"><h2 className="text-2xl font-black uppercase leading-none">{empName}</h2>{s.IsManual && <button onClick={()=>handleRemoveManual(s.Id)} className="text-[8px] text-red-500 font-bold px-2 py-1 rounded bg-red-50 hover:bg-red-500 hover:text-white transition-colors">Remove</button>}</div><div className="mt-2 text-left"><TimeDisplay s={s} logs={attendanceLogs} onEdit={(id: any, type: string) => setActiveForm({id, type})} /></div></div>
                        ...
                        // WEEKLY VIEW - FIXING FLOREN MATCHING
                        const isRowOpen = activeForm && staffShifts.some(sh => sh.Id === activeForm.id);
                        return (
                            <div key={empName} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 mb-6 print:mb-1 print:p-0 print:border-none print:shadow-none print:bg-transparent page-break-inside-avoid text-left text-left">
                                ...
                                {isRowOpen && activeForm && (
                                    <div className="mt-4 p-4 bg-slate-50 rounded-xl border-2 border-slate-200 print:hidden text-left text-left text-left">
                                       ...
                                       {(activeForm.type !== 'MARK') && (
                                            <div className="mt-4 p-5 bg-slate-800 rounded-2xl shadow-xl animate-in zoom-in-95 text-left text-left">
                                                ...
                                                : <textarea autoFocus placeholder="Reason..." className="w-full p-4 rounded-xl bg-slate-700 text-white h-24" value={formNote} onChange={e => setFormNote(e.target.value)} />}
                                                <div className="flex gap-2 mt-3 text-left text-left"><button disabled={isSaving} onClick={() => activeForm && submitAttendance(staffShifts.find(sh => sh.Id === activeForm.id), activeForm.type)} className="flex-1 bg-white text-slate-900 p-3 rounded-xl font-black uppercase text-xs">Save</button><button onClick={() => setActiveForm(null)} className="px-6 bg-slate-600 text-white rounded-xl font-bold text-xs uppercase">Cancel</button></div>
                                            </div>
                                       )}
                                    </div>
                                )}
