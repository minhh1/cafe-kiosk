...
export default function MartinPlaceDashboard() {
  ...
  // --- 4. IMPROVED LOCK LOGIC ---
  const isShiftDateLocked = (shiftDate: string) => {
    const now = new Date();
    // Get current Sydney date and hour
    const sydneyStr = now.toLocaleString("en-US", { timeZone: "Australia/Sydney", hour12: false });
    const sydneyDate = new Date(sydneyStr);
    const todayYMD = `${sydneyDate.getFullYear()}-${String(sydneyDate.getMonth() + 1).padStart(2, '0')}-${String(sydneyDate.getDate()).padStart(2, '0')}`;
    const currentHour = sydneyDate.getHours();

    if (shiftDate < todayYMD) return true; // It's a past day
    if (shiftDate === todayYMD && currentHour >= 18) return true; // It's today after 6pm
    return false; // Future or today before 6pm
  };

  const requestAuth = (shiftDate: string, action: () => Promise<void>) => {
    if (isShiftDateLocked(shiftDate)) {
      // DATE IS LOCKED: PIN is required to make a change
      setPendingAction(() => action);
      setShowPinModal(true);
    } else {
      // DATE IS OPEN: Execute immediately
      action();
    }
  };
  ...
  const fetchData = async (logsOnly = false) => {
    ...
    try {
      if (!logsOnly) {
        ...
        const formattedManual = (manualData || []).map(m => ({ Id: `manual-${m.id}`, IsManual: true, StartTime: m.start_time_str, EndTime: m.end_time_str, Team: m.team, Date: m.shift_date, _DPMetaData: { EmployeeInfo: { DisplayName: m.staff_name }, OperationalUnitInfo: { OperationalUnitName: m.team } } }));
  ...
  const handleConfirmCoffee = () => requestAuth(selectedDate, async () => {
    ...
    if (isCoffeeConfirmed) {
        ...
        const total = coffeeOperator === '+' ? std + (parseFloat(coffeeExtra)||0) : std - (parseFloat(coffeeExtra)||0);
        await supabase.from('daily_stats').upsert({ date: selectedDate, coffee_standard: std, coffee_extra: parseFloat(coffeeExtra)||0, coffee_operator: coffeeOperator, coffee_total: total, is_confirmed: true });
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 p-4 pb-20 font-sans print:bg-white print:p-0 text-left">
      <style jsx global>{` @media print { @page { size: A4 portrait; margin: 0.2cm; } body { background: white !important; font-size: 8pt; } .no-print, header { display: none !important; } } `}</style>
      
      {showPinModal && <PinModal onVerify={verifyPin} onClose={() => setShowPinModal(false)} />}
      ...
      <div className="max-w-5xl mx-auto print:max-w-none print:w-full">
        ...
        {viewMode === 'weekly' && (
          <div className="animate-in slide-in-from-top-4 duration-500">
            <div className="mb-4 bg-slate-800 text-white p-6 rounded-3xl shadow-lg flex justify-between items-center print:bg-white print:text-black print:p-2 print:border-b print:shadow-none print:rounded-none">
               ...
               <div className="text-right"><p className="text-[10px] font-black text-orange-400 uppercase mb-1">Weekly Coffee Total</p><p className="text-4xl font-black text-orange-500 print:text-black print:text-xl">{confirmedWeeklyCoffeeTotal.toFixed(1)} <span className="text-sm uppercase">KG</span></p></div>
            </div>
            <div className="grid grid-cols-7 gap-2 mb-8 px-2 print:hidden text-center">
                {weekRange.days.map((day, idx) => (
                    <div key={day} className="text-center"><p className="text-[8px] font-black text-slate-400 uppercase mb-1">{['Mon','Tue','Wed','Thu','Fri','Sat','Sun'][idx]}</p><div className="bg-white border border-slate-200 rounded-lg p-2 shadow-sm font-black text-xs text-slate-900">{getCoffeeForDate(day)}</div></div>
                ))}
            </div>
          </div>
        )}
        ...
        {[ {id: "FOH Team", name: "Front of House", col: "bg-slate-800"}, {id: "BOH Team", name: "Back of House", col: "bg-orange-600"} ].map(team => {
            ...
            const names = Array.from(new Set(teamShifts.map(s => s._DPMetaData.EmployeeInfo.DisplayName))).sort();
            return (
                                ...
                                return (
                                    <div key={s.Id} className={`bg-white p-5 rounded-2xl shadow-sm border mb-4 ${log ? STATUS_COLORS[log.action_type].border : 'border-slate-200'}`}>
                                        <div className="flex justify-between items-center gap-4">
                                            <div className="flex-1 text-left"><div className="flex items-center gap-3 text-left"><h2 className="text-2xl font-black uppercase leading-none">{s._DPMetaData.EmployeeInfo.DisplayName}</h2>{s.IsManual && <button onClick={()=>handleRemoveManual(s.Id, selectedDate)} className="text-[8px] text-red-500 font-bold px-2 py-1 rounded bg-red-50 hover:bg-red-500 hover:text-white transition-colors">Remove</button>}</div><div className="mt-2 text-left"><TimeDisplay s={s} logs={attendanceLogs} onEdit={(id: any, type: string) => setActiveForm({id, type, name: s._DPMetaData.EmployeeInfo.DisplayName})} /></div></div>
                                            <div className="flex flex-wrap gap-1 print:hidden text-left">{['ON_TIME', 'LATE', 'NO_SHOW', 'OTHER'].map(type => (
                                                <button key={type} onClick={() => { if(type==='NO_SHOW'||type==='OTHER') setActiveForm({id:s.Id, type, name:s._DPMetaData.EmployeeInfo.DisplayName}); else submitAttendance(s.Id.toString(), s._DPMetaData.EmployeeInfo.DisplayName, type, selectedDate, s.StartTime); }} className={`px-3 py-2 rounded-lg font-bold text-[9px] uppercase ${log?.action_type === type ? STATUS_COLORS[type].bg + " text-white scale-110" : "bg-slate-100 text-slate-400 opacity-40 hover:opacity-100"}`}>{type.replace('_',' ')}</button>
                                            ))}</div>
                                        </div>
        ...
        {/* --- GLOBAL ACTION MODAL --- */}
        {activeForm && (
            <div className="fixed inset-0 bg-black/40 z-[400] flex items-end sm:items-center justify-center p-0 sm:p-4 backdrop-blur-sm">
                <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full max-w-lg p-6 shadow-2xl animate-in slide-in-from-bottom duration-300 text-left text-left">
                    <div className="flex justify-between items-center mb-6 text-left text-left"><div><h4 className="font-black text-slate-800 uppercase tracking-widest text-left">{activeForm.type.replace('_',' ')}</h4><p className="text-[10px] font-bold text-slate-400 uppercase text-left">Updating {activeForm.name}</p></div><button onClick={() => setActiveForm(null)} className="bg-slate-100 h-10 w-10 rounded-full text-slate-400 font-bold text-xl">×</button></div>
                    <div className="flex flex-col gap-4 text-left">
                        {activeForm.type === 'MARK' && (<div className="grid grid-cols-2 gap-2">{['ON_TIME', 'LATE', 'NO_SHOW', 'OTHER'].map(type => (<button key={type} onClick={() => { if(type==='NO_SHOW'||type==='OTHER') setActiveForm({...activeForm, type}); else { const sh = shifts.find(sh => sh.Id.toString() === activeForm.id.toString()); submitAttendance(activeForm.id.toString(), activeForm.name || 'Staff', type, getLocalDateString(sh?.IsManual ? sh.Date : sh?.StartTime)); } }} className={`p-4 rounded-2xl font-black text-xs uppercase ${getLatestLog(activeForm.id)?.action_type === type ? STATUS_COLORS[type].bg + " text-white" : "bg-slate-100 text-slate-500"}`}>{type.replace('_',' ')}</button>))}</div>)}
                        {activeForm.type.startsWith('EDIT') && (<div className="bg-slate-50 p-4 rounded-2xl text-left"><label className="text-[10px] font-black uppercase text-slate-400 block mb-2 ml-1 text-left">Select New Time</label><input type="time" className="w-full p-4 rounded-xl bg-white border-2 border-slate-100 text-slate-900 font-black text-2xl outline-none" value={formTime} onChange={e => setFormTime(e.target.value)} /></div>)}
                        {['NO_SHOW', 'OTHER'].includes(activeForm.type) && (<textarea autoFocus placeholder="Mandatory Notes..." className="w-full p-4 rounded-xl bg-slate-50 border-2 border-slate-100 outline-none h-32 text-slate-900" value={formNote} onChange={e => setFormNote(e.target.value)} />)}
                        {activeForm.type !== 'MARK' && (<button disabled={isSaving} onClick={() => { const sh = shifts.find(sh => sh.Id.toString() === activeForm.id.toString()); submitAttendance(activeForm.id.toString(), activeForm.name || 'Staff', activeForm.type, getLocalDateString(sh?.IsManual ? sh.Date : sh?.StartTime), sh?.StartTime); }} className="w-full bg-slate-900 text-white p-5 rounded-2xl font-black uppercase text-xs shadow-lg">{isSaving ? 'Saving...' : 'Save Change'}</button>)}
                    </div>
                </div>
            </div>
        )}
