...
export default function ManagerDashboard() {
  ...
  const handleConfirmCoffee = async () => {
    ...
    const { error } = await supabase.from('daily_stats').upsert({ 
      date: selectedDate, 
      coffee_standard: std, coffee_extra: ext, coffee_operator: coffeeOperator,
      coffee_total: total, is_confirmed: true 
    });
    if (!error) { setIsCoffeeConfirmed(true); fetchData(); }
  };
    ...
    return (
      <div className={`${isCompact ? 'text-[10px]' : 'text-lg'} font-bold flex flex-wrap items-center gap-1`}>
        <button onClick={(e) => { e.stopPropagation(); setActiveForm({id: s.Id, type: 'EDIT_START'}); }} className="hover:underline text-left">
          {startLog ? <span className="text-blue-600">({startLog.override_start})</span> : <span className="text-orange-600">{origStart}</span>}
        </button>
        <span className="text-slate-300">—</span>
        <button onClick={(e) => { e.stopPropagation(); setActiveForm({id: s.Id, type: 'EDIT_END'}); }} className="hover:underline text-left">
          {endLog ? <span className="text-blue-600">({endLog.override_end})</span> : <span className="text-orange-600">{origEnd}</span>}
        </button>
      </div>
    );
  };
  ...
  const renderDailyCard = (s: any) => {
    ...
    return (
      <div key={s.Id} className={`bg-white p-5 rounded-2xl shadow-sm border mb-4 transition-all ${theme ? theme.border : 'border-slate-200'}`}>
        ...
        {log && theme && <div className={`mt-4 p-3 rounded-xl border text-xs ${theme.light} ${theme.text}`}><strong>{log.action_type.replace('_',' ')}:</strong> {log.notes}</div>}
        {renderEditForm(s)}
      </div>
    );
  };
            ...
            {staffShifts.sort((a,b) => a.StartTime - b.StartTime).map(s => {
              ...
              return (
                <div key={s.Id} onClick={() => setActiveForm({id: s.Id, type: 'MARK'})}
                  className={`p-3 rounded-xl border text-center flex flex-col justify-between min-h-[110px] cursor-pointer transition-all ${isSel ? 'ring-4 ring-orange-500/20 border-orange-500 shadow-md scale-105 z-10' : (log && theme ? `${theme.light} ${theme.border}` : 'bg-slate-50 border-slate-100')}`}>
                  <div>
      ...
      <div className="max-w-5xl mx-auto">
        <header className="bg-white p-6 rounded-3xl shadow-sm mb-8 border border-slate-200 flex flex-col lg:flex-row justify-between items-center gap-6">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            {/* COFFEE TRACKER - ONLY SHOW IN DAILY VIEW */}
            {viewMode === 'daily' && (
        ...
        {viewMode === 'weekly' && (
          <div className="mb-8 bg-slate-800 text-white p-6 rounded-3xl shadow-lg">
             <p className="text-[10px] font-black text-slate-400 uppercase mb-1">Business Week (Mon-Sun)</p>
