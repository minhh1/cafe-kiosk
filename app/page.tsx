...
export default function MartinPlaceDashboard() {
  ...
  const handleRemoveManual = async (id: any) => {
    if (confirm("Remove this temporary shift from view?")) {
        await supabase.from('manual_shifts').update({ status: 'removed', removed_at: new Date().toISOString() }).eq('id', id.toString().replace('manual-',''));
        fetchData();
    }
  };
  ...
  const submitAttendance = async (shift: any, type: string) => {
    ...
    if (!error && type === 'EDIT_START' && !shift.IsManual && formTime) {
      ...
      <div className="max-w-5xl mx-auto print:max-w-none print:w-full">
        ...
        {viewMode === 'weekly' && (
          <div className="animate-in slide-in-from-top-4 duration-500">
            ...
            <div className="flex justify-end gap-2 mb-4 print:hidden"><button onClick={exportToCSV} className="bg-slate-100 text-slate-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase">CSV Export</button><button onClick={() => window.print()} className="bg-orange-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase shadow-md">Print PDF</button></div>
          </div>
        )}
            ...
            <div className={`border-t-4 ${team.col.replace('bg-', 'border-')} pt-6 print:border-none print:pt-1`}>
                {Array.from(new Set(shifts.filter(s => s._DPMetaData?.OperationalUnitInfo?.OperationalUnitName === team.id).map(s => s._DPMetaData.EmployeeInfo.DisplayName))).map(empName => {
                    ...
                    if (viewMode === 'daily') {
                            ...
                            <div key={s.Id} className={`bg-white p-5 rounded-2xl shadow-sm border mb-4 ${attendanceLogs.find(l => l.shift_id === s.Id.toString()) ? STATUS_COLORS[attendanceLogs.find(l => l.shift_id === s.Id.toString()).action_type].border : 'border-slate-200'}`}>
                                ...
                                {activeForm && activeForm.id === s.Id && (
                                    <div className="mt-4 p-5 bg-slate-800 rounded-2xl shadow-xl animate-in zoom-in-95 text-left">
                                        ...
                                        : <textarea autoFocus placeholder="Mandatory Reason..." className="w-full p-4 rounded-xl" value={formNote} onChange={e => setFormNote(e.target.value)} />}
                        ...
                        // WEEKLY GRID VIEW
                        const isRowSel = activeForm && staffShifts.some(sh => sh.Id === activeForm.id);
                        return (
                            <div key={empName} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 mb-6 print:mb-1 print:p-0 print:border-none print:shadow-none print:bg-transparent page-break-inside-avoid text-left">
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-7 gap-3 mb-2 print:grid-cols-7 print:gap-0">
                                    {weekRange.days.map(dStr => {
                                        ...
                                        return (
                                            <div key={dStr} onClick={() => !isSaving && setActiveForm({id: dShift.Id, type: 'MARK'})} className={`p-3 rounded-xl border text-center flex flex-col justify-between min-h-[140px] cursor-pointer print:min-h-0 print:p-1 print:rounded-none ${activeForm?.id === dShift.Id ? 'ring-4 ring-orange-500 border-orange-500 z-10' : (dLog ? `${STATUS_COLORS[dLog.action_type].light} ${STATUS_COLORS[dLog.action_type].border}` : 'bg-slate-50 border-slate-100')}`}>
                                                <div>
                                                  <p className="text-[9px] font-black text-slate-400 uppercase leading-none mb-2 print:text-[5pt] print:mb-0">{new Date(dStr).toLocaleDateString('en-AU', { weekday: 'short', day: 'numeric' })}</p>
                                                  <TimeDisplay s={dShift} logs={attendanceLogs} isCompact onEdit={(id:any, type:string)=>setActiveForm({id, type})} />
                                                  {dShift.IsManual && <button onClick={(e)=>{e.stopPropagation(); handleRemoveManual(dShift.Id);}} className="text-[8px] text-red-400 font-bold mt-2 hover:text-red-600 print:hidden">Remove</button>}
                                                </div>
