...
export default function MartinPlaceDashboard() {
  ...
  const handleConfirmCoffee = useCallback(async () => {
    ...
    const { data: cData } = await supabase.from('daily_stats').select('*').gte('date', weekRange.mon).lte('date', weekRange.sun);
    if (cData) setCoffeeStats(cData);
    setIsCoffeeConfirmed(!isCoffeeConfirmed);
    setIsSaving(false);
  }, [selectedDate, coffeeStandard, coffeeExtra, coffeeOperator, isCoffeeConfirmed, weekRange.mon, weekRange.sun]);
  ...
  const submitAttendance = async (shift: any, type: string) => {
    ...
    const newTime = type.startsWith('EDIT') ? tFmt(formTime) : null;
      ...
      <div className="max-w-5xl mx-auto print:max-w-none print:w-full">
        <header className="bg-white p-6 rounded-3xl shadow-sm mb-8 border border-slate-200 flex flex-col lg:flex-row justify-between items-center gap-6 print:hidden text-left">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            {viewMode === 'daily' && (
                <CoffeeTracker standard={coffeeStandard} extra={coffeeExtra} operator={coffeeOperator} confirmed={isCoffeeConfirmed} onConfirm={handleConfirmCoffee} setStandard={setCoffeeStandard} setExtra={setCoffeeExtra} setOp={setCoffeeOperator} isSaving={isSaving} />
            )}
                            ...
                            <div key={empName} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 mb-6 print:mb-1 print:p-0 print:border-none print:shadow-none print:bg-transparent page-break-inside-avoid text-left">
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-7 gap-3 mb-2 print:grid-cols-7 print:gap-0">
                                    {weekRange.days.map(dStr => {
                                        ...
                                        return (
                                            <div key={dStr} onClick={() => !isSaving && setActiveForm({id: dShift.Id, type: 'MARK'})} className={`p-3 rounded-xl border text-center flex flex-col justify-between min-h-[140px] cursor-pointer print:min-h-0 print:p-1 print:rounded-none ${isDSelected ? 'ring-4 ring-orange-500 border-orange-500 z-10 scale-105' : (dLog ? `${STATUS_COLORS[dLog.action_type].light} ${STATUS_COLORS[dLog.action_type].border}` : 'bg-slate-50 border-slate-100')}`}>
                                                ...
                                                {dLog && (<div className={`mt-2 pt-2 border-t ${STATUS_COLORS[dLog.action_type].border} print:mt-0`}><p className={`text-[9px] font-black ${STATUS_COLORS[dLog.action_type].text} uppercase leading-none mb-1 print:text-[6pt]`}>{dLog.action_type}</p>{dLog.notes && <p className="text-[8px] text-slate-500 italic leading-tight line-clamp-2 print:text-[5pt] print:line-clamp-none">"{dLog.notes}"</p>}</div>)}
                                            </div>
                                        );
                                    })}
                                </div>
                                {isRowOpen && activeForm && (
                                    <div className="mt-4 p-4 bg-slate-50 rounded-xl border-2 border-slate-200 print:hidden text-left">
                                       <div className="flex flex-col sm:flex-row justify-between items-center gap-4"><p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Correction Menu</p>
