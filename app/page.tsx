...
export default function MartinPlaceDashboard() {
  ...
  const isShiftDateLocked = (shiftDate: string) => {
    const now = new Date();
    const sydneyLocale = now.toLocaleString("en-US", { timeZone: "Australia/Sydney", hour12: false });
    const sydneyDate = new Date(sydneyLocale);
    const sydneyTodayYMD = `${sydneyDate.getFullYear()}-${String(sydneyDate.getMonth() + 1).padStart(2, '0')}-${String(sydneyDate.getDate()).padStart(2, '0')}`;
    const sHour = sydneyDate.getHours();
    return shiftDate < sydneyTodayYMD || (shiftDate === sydneyTodayYMD && sHour >= 18);
  };
  ...
  const submitAttendance = (shiftId: string, name: string, type: string, shiftDate: string, origStart?: any) => requestAuth(shiftDate, async () => {
    ...
    setIsSaving(true);
    const tFmt = (t: string) => { if(!t.includes(':')) return t; const [h, m] = t.split(':'); return `${parseInt(h) % 12 || 12}:${m} ${parseInt(h) >= 12 ? 'pm' : 'am'}`; };
    const newTime = (type.startsWith('EDIT') || type === 'LATE') ? tFmt(formTime) : null;
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 p-4 pb-20 font-sans print:bg-white print:p-0 text-left">
      ...
      <div className="max-w-5xl mx-auto print:max-w-none print:w-full text-left">
        ...
        {viewMode === 'weekly' && (
          <div className="animate-in slide-in-from-top-4 duration-500 text-left">
            ...
            <div className="grid grid-cols-7 gap-2 mb-8 px-2 print:hidden text-center text-center">
                                    ...
                                    <div key={name} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 mb-6 print:mb-1 print:p-0 print:border-none print:shadow-none print:bg-transparent page-break-inside-avoid text-left text-left">
                                        ...
                                        <div className="grid grid-cols-7 gap-3 mb-2 print:grid-cols-7 print:gap-0">
                                            {weekRange.days.map(dStr => {
                                                ...
                                                if (!dShift) return <div key={dStr} className="min-h-[140px] bg-slate-50/20 border-slate-100 print:min-h-0 print:border-none text-left text-left"></div>;
            ...
            <div className="fixed inset-0 bg-black/40 z-[500] flex items-end sm:items-center justify-center p-0 sm:p-4 backdrop-blur-sm">
                <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full max-w-lg p-6 shadow-2xl animate-in slide-in-from-bottom duration-300 text-left text-left">
                    ...
                    <div className="flex flex-col gap-4 text-left text-left">
                        ...
                        {['LATE'].includes(activeForm.type) && (<textarea placeholder="Note (Optional)..." className="w-full p-4 rounded-xl bg-slate-50 border-2 border-slate-100 outline-none h-24 text-slate-900" value={formNote} onChange={e => setFormNote(e.target.value)} />)}
