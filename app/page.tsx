...
export default function MartinPlaceDashboard() {
  ...
  // --- 4. DATE-SPECIFIC TIME LOCK ---
  const isShiftDateLocked = (shiftDate: string) => {
    // 1. Get current Sydney time
    const sydneyStr = new Intl.DateTimeFormat("en-US", {
    ...
    const nowSydney = new Date(sydneyStr);
    const lockDeadline = new Date(`${shiftDate} 18:00:00`);

    if (nowSydney > lockDeadline) {
        alert(`LOCKED: Changes for ${shiftDate} were disabled at 6:00 PM on that date.`);
        return true;
    }
    return false;
  };
  ...
  const requestAuth = (shiftDate: string, action: () => Promise<void>) => {
    if (isShiftDateLocked(shiftDate)) return;
  ...
  const submitAttendance = async (shiftId: string, name: string, type: string, shiftDate: string, origStart?: any) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 p-4 pb-20 font-sans print:bg-white print:p-0 text-left">
      ...
      <div className="max-w-5xl mx-auto print:max-w-none print:w-full">
                                        ...
                                        <div className="flex justify-between items-center gap-4 text-left">
                                            ...
                                            <div className="flex flex-wrap gap-1 print:hidden">{['ON_TIME', 'LATE', 'NO_SHOW', 'OTHER'].map(type => (
                                                <button key={type} onClick={() => {
                                                    ...
                                                    if (type==='NO_SHOW'||type==='OTHER') setActiveForm({id: s.Id, type, name});
                                                    else submitAttendance(s.Id.toString(), name, type, selectedDate, s.StartTime);
                                                }} className={`px-3 py-2 rounded-lg font-bold text-[9px] uppercase ${log?.action_type === type ? STATUS_COLORS[type].bg + " text-white scale-110" : "bg-slate-100 text-slate-400 opacity-40 hover:opacity-100"}`}>{type.replace('_',' ')}</button>
                                            ))}</div>
                                        </div>
                                        {log && <div className={`mt-4 p-3 rounded-xl border text-xs ${STATUS_COLORS[log.action_type].light} ${STATUS_COLORS[log.action_type].text}`}><strong>{log.action_type.replace('_',' ')}:</strong> {log.notes}</div>}
                                    </div>
                                );
                            })
                        ) : (
        ...
        {/* MODALS (ManualShiftModal and activeForm Modal) ... */}
            ...
            <div className="fixed inset-0 bg-black/40 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 backdrop-blur-sm">
                <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full max-w-lg p-6 shadow-2xl animate-in slide-in-from-bottom duration-300 text-left">
                    ...
                    <div className="flex flex-col gap-4">
                        {activeForm.type.startsWith('EDIT') ? <input type="time" className="w-full p-4 rounded-xl bg-slate-50 border-2 border-slate-100 text-slate-900 font-black text-2xl outline-none" value={formTime} onChange={e => setFormTime(e.target.value)} /> 
                        : <textarea autoFocus placeholder="Enter notes here..." className="w-full p-4 rounded-2xl bg-slate-50 border-2 border-slate-100 outline-none h-32 text-slate-900" value={formNote} onChange={e => setFormNote(e.target.value)} />}
                        <button disabled={isSaving} onClick={() => {
                            const shift = shifts.find(sh => sh.Id.toString() === activeForm.id.toString());
                            submitAttendance(activeForm.id.toString(), activeForm.name || 'Staff', activeForm.type, getLocalDateString(shift?.IsManual ? shift.Date : shift?.StartTime), shift?.StartTime);
