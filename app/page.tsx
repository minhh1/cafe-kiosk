...
export default function MartinPlaceDashboard() {
  ...
  const confirmedWeeklyTotal = useMemo(() => coffeeStats.filter(c => c.is_confirmed).reduce((sum, c) => sum + (c.coffee_total || 0), 0), [coffeeStats]);
  ...
  const exportToCSV = () => {
    const headers = ["Date", "Name", "Start", "End", "Status", "Notes", "Daily Coffee"];
    const rows = shifts.map(s => {
      ...
      const coffee = coffeeStats.find(cs => cs.date === dStr && cs.is_confirmed)?.coffee_total || 0;
      return [new Date(s.StartTime * 1000 || s.Date).toLocaleDateString(), `"${s._DPMetaData?.EmployeeInfo?.DisplayName}"`, sE || "Sched", eE || "Sched", status?.action_type || "PENDING", `"${status?.notes || ""}"`, coffee].join(",");
    });
      ...
      <div className="max-w-5xl mx-auto print:max-w-none print:w-full">
        <header className="bg-white p-6 rounded-3xl shadow-sm mb-8 border border-slate-200 flex flex-col lg:flex-row justify-between items-center gap-6 print:hidden">
          <div className="flex-1 text-left"><h1 className="text-3xl font-black uppercase italic tracking-tighter">Martin Place Log</h1>
            <div className="flex gap-2 mt-4"><button onClick={() => setViewMode('daily')} className={`px-8 py-2.5 rounded-full text-xs font-black uppercase shadow-sm ${viewMode === 'daily' ? 'bg-slate-800 text-white scale-105' : 'bg-slate-100 text-slate-400'}`}>Daily</button><button onClick={() => setViewMode('weekly')} className={`px-8 py-2.5 rounded-full text-xs font-black uppercase shadow-sm ${viewMode === 'weekly' ? 'bg-slate-800 text-white scale-105' : 'bg-slate-100 text-slate-400'}`}>Weekly</button></div>
          </div>
          <div className="flex flex-col sm:flex-row items-center gap-4">
            ...
            <button onClick={() => { setNewManualShift({ name: '', start: '', end: '', team: 'FOH Team', date: selectedDate }); setShowManualModal(true); }} className="bg-slate-900 text-white px-5 py-3 rounded-2xl font-black text-[10px] uppercase shadow-lg">+ Create Shift</button>
        ...
        {viewMode === 'weekly' && (
          <div className="text-left animate-in slide-in-from-top-4">
            <div className="hidden print:block border-b-2 border-slate-900 pb-1 mb-2 text-left text-left">
                <h1 className="text-[12pt] font-black uppercase italic italic text-left">Martin Place Log</h1>
                <p className="text-[7pt] font-bold text-left">Week: {new Date(weekRange.mon).toLocaleDateString()} — {new Date(weekRange.sun).toLocaleDateString()} | Coffee Total: {confirmedWeeklyTotal.toFixed(1)}kg</p>
            ...
            <div className="mb-4 bg-slate-800 text-white p-6 rounded-3xl shadow-lg flex justify-between items-center print:hidden text-left">
               ...
               <div className="text-right"><p className="text-[10px] font-black text-orange-400 uppercase mb-1">Weekly Coffee Total</p><p className="text-4xl font-black text-orange-500">{confirmedWeeklyTotal.toFixed(1)} <span className="text-sm">KG</span></p></div>
            </div>
            ...
            return (
                <div key={team.id} className="mb-10 text-left">
                    ...
                    <div className={`border-t-4 ${team.col.replace('bg-', 'border-')} pt-6 print:border-none print:pt-1`}>
                        {viewMode === 'daily' ? teamShifts.filter(s => (getLocalDateString(s.IsManual ? s.Date : s.StartTime) === selectedDate)).map(s => {
                            const log = getLatestLog(s.Id);
                            const isOpen = activeForm?.id === s.Id;
                            return (
                                <div key={s.Id} className={`bg-white p-5 rounded-2xl shadow-sm border mb-4 ${log ? STATUS_COLORS[log.action_type].border : 'border-slate-200'}`}>
                                    <div className="flex justify-between items-center gap-4">
                                        ...
                                        <div className="flex flex-wrap gap-1 print:hidden">{['ON_TIME', 'LATE', 'NO_SHOW', 'OTHER'].map(type => (
                                            <button key={type} onClick={() => { if(type==='NO_SHOW'||type==='OTHER') setActiveForm({id:s.Id, type, name:s._DPMetaData.EmployeeInfo.DisplayName}); else submitAttendance(s.Id.toString(), s._DPMetaData.EmployeeInfo.DisplayName, type, s.StartTime); }} className={`px-3 py-2 rounded-lg font-bold text-[9px] uppercase ${log?.action_type === type ? STATUS_COLORS[type].bg + " text-white scale-110" : "bg-slate-100 text-slate-400 opacity-40 hover:opacity-100"}`}>{type.replace('_',' ')}</button>
                                        ))}</div>
                                    </div>
                            ...
                            return (
                                <div key={name} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 mb-6 print:mb-1 print:p-0 print:border-none print:shadow-none print:bg-transparent page-break-inside-avoid text-left text-left">
        ...
        {activeForm && (
            <div className="fixed inset-0 bg-black/40 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 backdrop-blur-sm">
                <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full max-w-lg p-6 shadow-2xl animate-in slide-in-from-bottom duration-300 text-left">
                    <div className="flex justify-between items-center mb-6"><div><h4 className="font-black text-slate-800 uppercase tracking-widest">{activeForm.type.replace('_',' ')}</h4><p className="text-[10px] font-bold text-slate-400 uppercase">Updating {activeForm.name}</p></div><button onClick={() => setActiveForm(null)} className="bg-slate-100 h-10 w-10 rounded-full text-slate-400 font-bold text-xl">×</button></div>
                    <div className="flex flex-col gap-4">
                        {activeForm.type === 'MARK' && (<div className="grid grid-cols-2 gap-2">{['ON_TIME', 'LATE', 'NO_SHOW', 'OTHER'].map(type => (<button key={type} onClick={() => { if(type==='NO_SHOW'||type==='OTHER') setActiveForm({...activeForm, type}); else submitAttendance(activeForm.id.toString(), activeForm.name || 'Staff', type); }} className={`p-4 rounded-2xl font-black text-xs uppercase ${getLatestLog(activeForm.id)?.action_type === type ? STATUS_COLORS[type].bg + " text-white" : "bg-slate-100 text-slate-500"}`}>{type.replace('_',' ')}</button>))}</div>)}
                        {activeForm.type.startsWith('EDIT') && (<div className="bg-slate-50 p-4 rounded-2xl"><label className="text-[10px] font-black uppercase text-slate-400 block mb-2 ml-1">Select New Time</label><input type="time" className="w-full p-4 rounded-xl bg-white border-2 border-slate-100 text-slate-900 font-black text-2xl outline-none" value={formTime} onChange={e => setFormTime(e.target.value)} /></div>)}
                        {['NO_SHOW', 'OTHER'].includes(activeForm.type) && (<textarea autoFocus placeholder="Mandatory Notes..." className="w-full p-4 rounded-2xl bg-slate-50 border-2 border-slate-100 outline-none h-32 text-slate-900" value={formNote} onChange={e => setFormNote(e.target.value)} />)}
                        {activeForm.type !== 'MARK' && (<button disabled={isSaving} onClick={() => submitAttendance(activeForm.id.toString(), activeForm.name || 'Staff', activeForm.type)} className="w-full bg-slate-900 text-white p-5 rounded-2xl font-black uppercase text-xs shadow-lg">{isSaving ? '...' : 'Save Change'}</button>)}
                    </div>
                </div>
            </div>
        )}
