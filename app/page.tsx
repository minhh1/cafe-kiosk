...
export default function MartinPlaceDashboard() {
  ...
  // --- 4. NEW 1-WEEK SECURITY LOGIC ---
  const isShiftLocked = (shiftDate: string) => {
    const now = new Date();
    // Find this week's Monday (Sydney Time)
    ...
    // Lock if the shift date is before the current business week's Monday
    return checkDate < thisMonday;
  };

  const requestAuth = (shiftDate: string, action: () => Promise<void>) => {
    if (isShiftLocked(shiftDate)) {
      // Shift is from a previous week: PIN REQUIRED
      setPendingAction(() => action);
      setShowPinModal(true);
    } else {
      // Shift is current week or future: NO PIN REQUIRED
      action();
    }
  };
  ...
  const submitAttendance = (shiftId: string, name: string, type: string, shiftDate: string, origStart?: any) => {
    requestAuth(shiftDate, async () => {
            ...
            <div className="grid grid-cols-7 gap-2 mb-8 px-2 print:hidden text-center">
                {weekRange.days.map((day, idx) => (
                    <div key={day} className="text-center">
                        ...
                        <div className="bg-white border border-slate-200 rounded-lg p-2 shadow-sm font-black text-xs text-slate-900">{getCoffeeForDate(day)}</div>
                    </div>
                ))}
            </div>
            <div className="flex justify-end gap-2 mb-4 print:hidden text-left"><button onClick={exportToCSV} className="bg-slate-100 text-slate-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase hover:bg-slate-200">CSV</button><button onClick={() => window.print()} className="bg-orange-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase shadow-md">Print PDF</button></div>
          </div>
        )}
                                    ...
                                    <div key={s.Id} className={`bg-white p-5 rounded-2xl shadow-sm border mb-4 ${log ? STATUS_COLORS[log.action_type].border : 'border-slate-200'}`}>
                                        <div className="flex justify-between items-center gap-4 text-left">
                                            ...
                                            <div className="flex flex-wrap gap-1 print:hidden">{['ON_TIME', 'LATE', 'NO_SHOW', 'OTHER'].map(type => (
                                                <button key={type} onClick={() => { if(type==='LATE'||type==='NO_SHOW'||type==='OTHER') setActiveForm({id:s.Id, type, name:s._DPMetaData.EmployeeInfo.DisplayName}); else submitAttendance(s.Id.toString(), s._DPMetaData.EmployeeInfo.DisplayName, type, selectedDate, s.StartTime); }} className={`px-3 py-2 rounded-lg font-bold text-[9px] uppercase ${log?.action_type === type ? STATUS_COLORS[type].bg + " text-white scale-110 shadow-md" : "bg-slate-100 text-slate-400 opacity-40 hover:opacity-100"}`}>{type.replace('_',' ')}</button>
                                            ))}</div>
                                        </div>
