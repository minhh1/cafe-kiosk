...
export default function ManagerDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      const rosterRes = await fetch(`/api/roster?startDate=${viewMode === 'daily' ? selectedDate : range.mon}&endDate=${viewMode === 'daily' ? selectedDate : range.sun}`);
      ...
      setShifts(Array.isArray(rosterData) ? rosterData : []);
  ...
  const submitAttendance = async (shift: any, type: string) => {
    // Note only mandatory for No Show and Other
    if ((type === 'OTHER' || type === 'NO_SHOW') && !formNote.trim()) {
  ...
  const deleteCorrection = async (shiftId: any, type: string) => {
    setIsSaving(true);
    await supabase.from('attendance_logs').delete().match({ shift_id: shiftId.toString(), action_type: type });
    setActiveForm(null);
    await fetchData();
    setIsSaving(false);
  };
  ...
  const exportToCSV = () => {
    const headers = ["Date", "Name", "Scheduled Start", "Scheduled End", "Actual Start", "Actual End", "Status", "Notes", "Daily Coffee (kg)"];
    const rows = shifts.map(s => {
      ...
      const status = attendanceLogs.find(l => l.shift_id.toString() === s.Id.toString() && !l.action_type.startsWith('EDIT'));
      const dateStr = new Date(s.StartTime * 1000).toISOString().split('T')[0];
      const coffee = coffeeStats.find(c => c.date === dateStr && c.is_confirmed)?.coffee_total || 0;
      ...
      return [new Date(s.StartTime * 1000).toLocaleDateString(), `"${s._DPMetaData?.EmployeeInfo?.DisplayName}"`, origS, origE, sE?.override_start || origS, eE?.override_end || origE, status?.action_type || "PENDING", `"${(status?.notes || "").replace(/"/g, '""')}"`, coffee.toFixed(1)].join(",");
    });
    const csvContent = "data:text/csv;charset=utf-8," + [headers.join(","), ...rows].join("\n");
    const link = document.createElement("a");
    link.setAttribute("href", encodeURI(csvContent));
    link.setAttribute("download", `Martin_Place_Log_${selectedDate}.csv`);
    ...
    return (
      <div className={`${isCompact ? 'text-[9px] print:text-[7pt]' : 'text-lg'} font-bold flex flex-wrap items-center gap-1 print:text-black`}>
        <button onClick={(e) => { e.stopPropagation(); setActiveForm({id: s.Id, type: 'EDIT_START'}); }} className="hover:underline">
          {sEdit ? <><span className="line-through text-slate-300 decoration-slate-400">{oS}</span> <span className="text-blue-600 font-black">({sEdit.override_start})</span></> : <span className="text-orange-600">{oS}</span>}
        </button>
        <span className="text-slate-300">—</span>
        <button onClick={(e) => { e.stopPropagation(); setActiveForm({id: s.Id, type: 'EDIT_END'}); }} className="hover:underline">
          {eEdit ? <><span className="line-through text-slate-300 decoration-slate-400">{oE}</span> <span className="text-blue-600 font-black">({eEdit.override_end})</span></> : <span className="text-orange-600">{oE}</span>}
        </button>
      </div>
    );
  };
  ...
  const renderEditForm = (s: any) => {
    ...
    const existing = attendanceLogs.find(l => l.shift_id.toString() === s.Id.toString() && l.action_type === activeForm.type);
    const isLate = activeForm.type === 'LATE';
    ...
    return (
      <div className="mt-4 p-5 bg-slate-800 rounded-2xl shadow-xl animate-in zoom-in-95 text-left print:hidden">
        ...
        <div className="flex flex-col gap-3">
          ...
          : <textarea autoFocus placeholder={isLate ? "Notes (Optional)..." : "Reason (Mandatory)..."} className="p-4 rounded-xl bg-slate-700 text-white h-20 outline-none" value={formNote} onChange={e => setFormNote(e.target.value)} />}
          <div className="flex gap-2">
            <button disabled={isSaving} onClick={() => submitAttendance(s, activeForm.type)} className="flex-1 bg-white text-slate-900 p-3 rounded-xl font-black uppercase text-xs">Save</button>
            {existing && <button disabled={isSaving} onClick={() => deleteCorrection(s.Id, activeForm.type)} className="bg-red-600 text-white px-4 py-3 rounded-xl font-black text-xs uppercase">Delete</button>}
    ...
    return Object.entries(grouped).map(([name, staffShifts]) => {
      ...
      return (
        <div key={name} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 mb-6 print:mb-1 print:p-0 print:border-none print:shadow-none print:bg-transparent page-break-inside-avoid">
          <h3 className="text-xl font-black text-slate-800 uppercase border-b pb-3 mb-4 tracking-tighter print:text-[8pt] print:mb-0.5 print:pb-0.5 print:text-left">{name}</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-7 gap-3 mb-2 print:grid-cols-7 print:gap-0">
            {[...staffShifts].sort((a,b) => a.StartTime - b.StartTime).map(s => {
              ...
              return (
                <div key={s.Id} onClick={() => !isSaving && setActiveForm({id: s.Id, type: 'MARK'})}
                  className={`p-3 rounded-xl border text-center flex flex-col justify-between min-h-[140px] cursor-pointer transition-all print:min-h-0 print:p-1 print:rounded-none ${activeForm?.id === s.Id ? 'ring-4 ring-orange-500/20 border-orange-500 z-10' : (log && theme ? `${theme.light} ${theme.border}` : 'bg-slate-50 border-slate-100')}`}>
                  <div>
                  ...
                  {log && theme && (
                    <div className={`mt-2 pt-2 border-t ${theme.border} print:mt-0`}>
          ...
          {activeInRow && (
            <div className="mt-4 p-4 bg-slate-50 rounded-xl border-2 border-slate-200 print:hidden">
               <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                  <p className="text-[10px] font-black text-slate-500 uppercase text-left">Action Menu</p>
            ...
            {viewMode === 'daily' && (
              <div className={`p-3 rounded-2xl flex items-center gap-3 transition-colors ${isCoffeeConfirmed ? 'bg-green-50 border-2 border-green-200' : 'bg-orange-50 border border-orange-200'}`}>
                <div className="flex flex-col items-center">
                    ...
                    <input disabled={isCoffeeConfirmed || isSaving} type="number" placeholder="10" value={coffeeStandard} onChange={e => {setCoffeeStandard(e.target.value); setIsCoffeeConfirmed(false);}} className="w-12 p-1 rounded-lg text-center bg-white font-bold border-none outline-none" />
                </div>
                <button disabled={isCoffeeConfirmed || isSaving} onClick={() => {setCoffeeOperator(coffeeOperator === '+' ? '-' : '+'); setIsCoffeeConfirmed(false);}} className={`w-8 h-8 rounded-full font-black text-lg flex items-center justify-center transition-colors ${coffeeOperator === '+' ? 'bg-orange-500 text-white' : 'bg-blue-500 text-white'}`}>{coffeeOperator}</button>
                <div className="flex flex-col items-center">
                    <span className="text-[8px] font-black text-slate-400 uppercase">Extra</span>
                    <input disabled={isCoffeeConfirmed || isSaving} type="number" value={coffeeExtra} placeholder="0" onChange={e => {setCoffeeExtra(e.target.value); setIsCoffeeConfirmed(false);}} className="w-12 p-1 rounded-lg text-center bg-white font-bold border-none outline-none" />
                </div>
          ...
          <div className="animate-in slide-in-from-top-4 duration-500">
            ...
            <div className="mb-4 bg-slate-800 text-white p-6 rounded-3xl shadow-lg flex justify-between items-center print:hidden">
               <div className="text-left">
                  <p className="text-[10px] font-black text-slate-400 uppercase mb-1">Business Week</p>
                  <p className="text-lg font-bold">{new Date(weekRange.mon).toLocaleDateString()} — {new Date(weekRange.sun).toLocaleDateString()}</p>
               </div>
               <div className="text-right">
                  <p className="text-[10px] font-black text-orange-400 uppercase mb-1">Total Weekly Coffee</p>
                ...
                {['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map((day, idx) => {
                    ...
                    return (
                        <div key={day} className="text-center">
                            ...
                            <div className="bg-white border border-slate-200 rounded-lg p-2 shadow-sm font-black text-xs text-slate-700">{getCoffeeForDate(dateStr)}</div>
                        </div>
                    )
                })}
            </div>
            <div className="flex justify-end gap-2 mb-4 print:hidden">
                ...
                <button onClick={() => window.print()} className="bg-orange-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase shadow-md">Print Report</button>
            </div>
          </div>
        )}
