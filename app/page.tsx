...
export default function MartinPlaceDashboard() {
  ...
  const isLocked = (shiftDate: string) => {
    const now = new Date();
    ...
    return shiftDate < todayYMD || (shiftDate === todayYMD && sydneyDate.getHours() >= 18);
  };
  ...
  const verifyPin = async (enteredPin: string) => {
    ...
    if (data?.pin === enteredPin) { setShowPinModal(false); await pendingAction(); setPendingAction(null); } 
  ...
  const submitAttendance = (shiftId: string, name: string, type: string, shiftDate: string, origStart?: any) => requestAuth(shiftDate, async () => {
    ...
    setIsSaving(true);
    const newTime = (type.startsWith('EDIT') || type === 'LATE') ? format24to12(formTime) : null;
    const origS = typeof origStart === 'number' ? formatTime(origStart) : origStart;
    ...
    if (type === 'EDIT_START' && origStart && formTime) {
        ...
        } else if (newTime === origS) {
            await supabase.from('attendance_logs').insert({ shift_id: shiftId, staff_name: name, action_type: 'ON_TIME', notes: "" });
        }
    }
  ...
  const exportToCSV = () => {
    ...
    const rows = shifts.map(s => {
      ...
      const sE = logs.find(l => l.action_type === 'EDIT_START' || (l.action_type === 'LATE' && l.override_start))?.override_start;
      ...
      return [new Date(s.StartTime * 1000 || s.Date).toLocaleDateString(), `"${s._DPMetaData.EmployeeInfo.DisplayName}"`, sE || "Sched", eE || "Sched", status?.action_type || "PENDING", `"${status?.notes || ""}"`, (coffeeStats.find(cs => cs.date === dStr && cs.is_confirmed)?.coffee_total || 0).toFixed(1)].join(",");
    });
      ...
      <div className="max-w-5xl mx-auto print:max-w-none print:w-full">
        ...
        <header className="bg-white p-6 rounded-3xl shadow-sm mb-8 border border-slate-200 flex flex-col lg:flex-row justify-between items-center gap-6 print:hidden">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 text-left">
            ...
            <button onClick={() => setShowManualModal(true)} className="bg-slate-900 text-white px-5 py-3 rounded-2xl font-black text-[10px] uppercase shadow-lg hover:bg-orange-600 transition-colors">+ Create Shift</button>
        ...
        {viewMode === 'weekly' && (
          <div className="animate-in slide-in-from-top-4 duration-500 text-left">
            <div className="hidden print:block border-b-2 border-slate-900 pb-1 mb-2">
                ...
                <p className="text-[7pt] font-bold">Week: {new Date(weekRange.mon).toLocaleDateString()} — {new Date(weekRange.sun).toLocaleDateString()} | Total Coffee: {confirmedWeeklyCoffeeTotal.toFixed(1)}kg</p>
                <div className="flex gap-3 mt-1 text-[6pt] font-black text-slate-500 uppercase">{weekRange.days.map((d, i) => <span key={d}>{['Mon','Tue','Wed','Thu','Fri','Sat','Sun'][i]}: {(coffeeStats.find(cs => cs.date === d && cs.is_confirmed)?.coffee_total || 0).toFixed(1)}kg</span>)}</div>
            </div>
            <div className="mb-4 bg-slate-800 text-white p-6 rounded-3xl shadow-lg flex justify-between items-center print:hidden text-left">
               ...
               <div className="text-right"><p className="text-[10px] font-black text-orange-400 uppercase mb-1">Weekly Total</p><p className="text-4xl font-black text-orange-500">{confirmedWeeklyCoffeeTotal.toFixed(1)} <span className="text-sm uppercase">KG</span></p></div>
            </div>
                    ...
                    <div className={`border-t-4 ${team.col.replace('bg-', 'border-')} pt-6 print:border-none print:pt-1`}>
                        {viewMode === 'daily' ? (
                                        ...
                                        <div className="flex justify-between items-center gap-4 text-left">
                                            ...
                                            <div className="flex flex-wrap gap-1 print:hidden">{['ON_TIME', 'LATE', 'NO_SHOW', 'OTHER'].map(type => (
                                                <button key={type} onClick={() => { 
                                                    ...
                                                    if(['LATE','NO_SHOW','OTHER'].includes(type)) setActiveForm({id:s.Id, type, name}); 
                                                    else submitAttendance(s.Id.toString(), name, type, selectedDate, s.StartTime); 
                            ...
                            distinctNames.map(name => {
                                ...
                                const rowIsActive = activeForm && staffShifts.some(sh => sh.Id === activeForm.id);
                                return (
                                    <div key={name} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 mb-6 print:mb-1 print:p-0 print:border-none print:shadow-none print:bg-transparent page-break-inside-avoid text-left">
                                        ...
                                        <div className="grid grid-cols-7 gap-3 mb-2 print:grid-cols-7 print:gap-0">
                                            {weekRange.days.map(dStr => {
                                                ...
                                                if (!dShift) return <div key={dStr} className="min-h-[140px] bg-slate-50/20 border-slate-100 print:min-h-0 print:border-none text-left"></div>;
                                        ...
                                        {rowIsActive && activeForm && activeForm.type !== 'MARK' && (
                                            <div className="mt-4 p-4 bg-slate-50 rounded-xl border-2 border-slate-200 print:hidden text-left">
                                               ...
                                               <div className="flex flex-wrap gap-1 text-left text-left">{['ON_TIME', 'LATE', 'NO_SHOW', 'OTHER'].map(type => (
                                                <button key={type} onClick={() => (type === 'NO_SHOW' || type === 'OTHER') ? setActiveForm({id: activeForm.id, type, name}) : submitAttendance(activeForm.id.toString(), name, type, getLocalDateString(staffShifts.find(sh => sh.Id === activeForm.id)?.IsManual ? staffShifts.find(sh => sh.Id === activeForm.id)?.Date : staffShifts.find(sh => sh.Id === activeForm.id)?.StartTime))} className={`px-3 py-2 rounded-lg font-bold text-[9px] uppercase ${getLatestLog(activeForm.id)?.action_type === type ? STATUS_COLORS[type].bg + " text-white scale-110 shadow-lg" : "bg-white text-slate-400 opacity-40 hover:opacity-100 shadow-sm"}`}>{type.replace('_',' ')}</button>
                                               ))}</div></div>
                                            </div>
                                        )}
                                    </div>
        ...
        {activeForm && activeForm.type !== 'MARK' && (
            <div className="fixed inset-0 bg-black/40 z-[500] flex items-end sm:items-center justify-center p-0 sm:p-4 backdrop-blur-sm">
                <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full max-w-lg p-6 shadow-2xl animate-in slide-in-from-bottom duration-300 text-left">
                    <div className="flex justify-between items-center mb-6 text-left text-left"><div><h4 className="font-black text-slate-800 uppercase tracking-widest text-left">{activeForm.type.replace('_',' ')} DETAILS</h4><p className="text-[10px] font-bold text-slate-400 uppercase text-left">Updating {activeForm.name}</p></div><button onClick={() => setActiveForm(null)} className="bg-slate-100 h-10 w-10 rounded-full text-slate-400 font-bold text-xl">×</button></div>
                    <div className="flex flex-col gap-4 text-left">
                        {(activeForm.type.startsWith('EDIT') || activeForm.type === 'LATE') && (<div className="bg-slate-50 p-4 rounded-2xl text-left"><label className="text-[10px] font-black uppercase text-slate-400 block mb-2 ml-1 text-left">Select Arrived/Corrected Time</label><input type="time" className="w-full p-4 rounded-xl bg-white border-2 border-slate-100 text-slate-900 font-black text-2xl outline-none" value={formTime} onChange={e => setFormTime(e.target.value)} /></div>)}
